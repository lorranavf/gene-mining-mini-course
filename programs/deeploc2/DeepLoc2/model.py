...
class ProtT5E2E(pl.LightningModule):
    ...
    def forward(self, toks, lens, non_mask):#, dct_mat, idct_mat):
        # in lightning, forward defines the prediction/inference actions
        x = self.embedding_func(input_ids=torch.tensor(toks['input_ids']),attention_mask=torch.tensor(toks['attention_mask'])).last_hidden_state.cpu()[:, :-1].float()
        x_loc_preds, x_signal_preds, x_attnss = [], [], []
        for i in range(5):
          x_pred, x_pool, x_attns = self.subcel_clfs[i].predict(x, lens, non_mask[:, :-1])
        ...
        return torch.stack(x_loc_preds).mean(0).numpy(), torch.stack(x_attnss).mean(0).numpy(), torch.stack(x_signal_preds).mean(0).numpy()
...
class ESM1bE2E(pl.LightningModule):
    ...
    def forward(self, toks, lens, non_mask):#, dct_mat, idct_mat):
        # in lightning, forward defines the prediction/inference actions
        x = self.embedding_func(toks, repr_layers=[33])["representations"][33][:, 1:-1].float()
        x_loc_preds, x_signal_preds, x_attnss = [], [], []
        for i in range(5):
          x_pred, x_pool, x_attns = self.subcel_clfs[i].predict(x, lens, non_mask[:, 1:-1])
          x_loc_preds.append(torch.sigmoid(x_pred))
          x_attnss.append(x_attns)
          x_signal_pred = torch.sigmoid(self.signaltype_clfs[i](torch.cat((x_pool, torch.sigmoid(x_pred)), dim=1)))
          x_signal_preds.append(x_signal_pred)
